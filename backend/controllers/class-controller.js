...
const getSclassDetail = async (req, res) => {
    try {
        ...
        if (sclass) {
            sclass = await sclass.populate("school", "schoolName","studentFee")
            res.send(sclass);
        }
        else {
...
const updateSclass = async (req, res) => {
    try {
        const { id } = req.params;
        const updatedClass = await Sclass.findByIdAndUpdate(id, req.body, { new: true });
